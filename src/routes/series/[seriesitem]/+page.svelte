<script lang="ts">
  ...
  import { Card, Text, Badge, Box, Group, Image} from '@svelteuidev/core';
  ...
  const filterElements = () => {
    filteredElements = elementsdata.filter(element => {
      return (!category || element.category === category) &&
...
<style>
  ...
  .filter-panel select, .filter-panel input[type="radio"] {
    margin-right: 10px;
  }
...
{#if !error}
  <h1 style="text-align: center; margin-bottom: 20px; color:white">{property} of Elements in the Periodic Table</h1>
  ...
  Sort Order
      <label>
        <input type="radio" name="sortOrder" value="asc" checked={sortOrder === 'asc'} on:change={() => {sortOrder = 'asc'; filterElements();}}>
        Ascending
      </label>
      <label>
        <input type="radio" name="sortOrder" value="desc" checked={sortOrder === 'desc'} on:change={() => {sortOrder = 'desc'; filterElements();}}>
        Descending
      </label>
    </div>
  </div>
  ...
  >
    {#each filteredElements as element (element.symbol)}
      <Card shadow="sm" padding="sm" m="sm" style="width: 300px;">
        <Card.Section>
          <Image src={element.image?.url || element.bohr_model_image} alt={element.name} width="300px" height="300px" />
        </Card.Section>
        ...
        <Text size="sm">
          <strong>Discovered by:</strong> {element.discovered_by || 'Unknown'}
        </Text>

        <Text size="sm">
          <strong>{property}:</strong> {element[property]}
        </Text>
        ...
        <Text size="sm">
          <strong>Phase at STP:</strong> {element.phase}
        </Text>

        <Card.Section style="margin-top: 15px;">
          <Text size="xs" color="dimmed" align="center">
            <a href={element.source} target="_blank">More Information</a>
          </Text>
        </Card.Section>
      </Card>
    {/each}
